...
(function () {
  ...
  /* Ping the local server to determine real connectivity.
     navigator.onLine is unreliable on emulators / captive-portal networks
     that have LAN access but no internet. We consider "online" as long as
     the SHAMEL server itself is reachable. */
  let _serverReachable = null; // null = unknown, true/false after first check

  function pingServer(callback) {
    const url = '/api/ping/?_=' + Date.now();
  ...
  function updateOnlineState() {
    ...
    updateCornerIndicator('checking');
    pingServer(function(reachable) {
  ...
  /* Re-check server reachability every 15 s */
  setInterval(updateOnlineState, 15000);
